...
This package helps to detect different call states like `Incoming`, `Disconnected`, `Dialing` and `Connected` for iOS. 
For android, this package will give following states, `Connected`, `Incoming`, `IDLE` `Disconnected` and `Missed`. In the case of `Incoming` for android, the package will also provide with the incoming phone number.
...
startListenerTapped() {
	this._subscription = CallDetectorManager.addListener("PhoneState", ({state, incomingNumber, status, prevStatus}) => {

		if (state === 'Disconnected') {
			// Do something call got disconnected
		} else if(state === 'IDLE') {
			// Do something call free
	...
	await CallDetectorManager.startListener();

	currentState = await CallDetectorManager.getCallState();
}

stopListenerTapped() {
	this._subscription && this._subscription.remove();
	await CallDetectorManager.stopListener()
}
